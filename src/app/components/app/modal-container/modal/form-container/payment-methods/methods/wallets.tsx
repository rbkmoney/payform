...
export const Wallets: React.SFC<MethodProps> = (props) => (
    <li className={method} onClick={toWallets.bind(null, props)} id="wallets-payment-method">
        <WalletsIcon />
        <div className={text}>
            <h5 className={title}>{props.locale['form.payment.method.name.wallet.label']}</h5>
            <p className={description}>{props.locale['form.payment.method.description.qiwi.text']}</p>
        </div>
    </li>
);
