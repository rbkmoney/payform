...
const checkAmountCapableForTemplateCostRange = (amount: number, cost: InvoiceTemplateLineCostRange): CheckResult => {
    ...
    return {
        ...
        message: available
            ? null
            : `Value of param 'amount' should be in range from ${lowerBound} to ${upperBound}. Current amount value: ${amount}.`
    };
};
    ...
    switch (price.costType) {
        case CostType.InvoiceTemplateLineCostFixed:
            return {
                ...
                message:
                    "Param 'amount' is only available for invoice templates with range or unlim cost types. Current invoice template has fixed cost type."
            };
    ...
    switch (t.details.templateType) {
        case TemplateType.InvoiceTemplateMultiLine:
            return {
                ...
                reason: UnavailableReason.capability,
                message:
                    "Param 'amount' is only available for single line invoice templates. Current invoice template has multi line type."
            };
