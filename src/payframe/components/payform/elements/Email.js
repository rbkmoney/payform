...
import { setEmailVal } from '../../../../redux/actions/viewDataActions';
...
class Email extends React.Component {
    ...
    componentDidMount() {
        ...
        this.input.value = this.props.email.value;
    }

    componentWillReceiveProps(nextProps) {
        const classList = this.input.parentNode.classList;
        if (nextProps.email.valid === false) {
    ...
    handleChange(event) {
        this.props.actions.setEmailVal(event.target.value);
    }
    ...
    render() {
        return <div className="payform--group payform--email">
            <input
                ...
                placeholder={this.props.locale['input.payment.email.placeholder']}
                autoComplete="on"
                autoCorrect="no"
                autoCapitalize="no"
                spellCheck="no"/>
...
function mapStateToProps(state) {
    return {
        locale: state.locale,
        email: state.viewData.cardForm.email
    };
}
...
function mapActionsToProps(dispatch) {
    return {
        actions: {
            setEmailVal: bindActionCreators(setEmailVal, dispatch)
        }
    };
}

export default connect(mapStateToProps, mapActionsToProps)(Email);
