...
import { setCardHolderVal } from '../../../../redux/actions/viewDataActions';
...
class CardHolder extends React.Component {
    ...
    componentDidMount() {
        ...
        this.input.value = this.props.cardHolder.value;
    }

    componentWillReceiveProps(nextProps) {
        const classList = this.input.parentNode.classList;
        if (nextProps.cardHolder.valid === false) {
    ...
    handleChange(event) {
        this.props.actions.setCardHolderVal(event.target.value);
    }
        ...
        return (
            <div className="payform--group payform--card-holder">
                <input
                    ...
                    placeholder={this.props.locale['input.payment.cardHolder.placeholder']}
                    autoComplete="off"
                    autoCorrect="no"
                    autoCapitalize="no"
                    spellCheck="no"/>
...
function mapActionsToProps(dispatch) {
    return {
        actions: {
            setCardHolderVal: bindActionCreators(setCardHolderVal, dispatch)
        }
    };
}

export default connect(mapStateToProps, mapActionsToProps)(CardHolder);
